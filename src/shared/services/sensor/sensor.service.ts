...
export class SensorService {
  constructor(
    @InjectModel('Sensor') private sensorModel: Model<SensorModel>,
    private httpService: HttpService,
    private hostService: HostService,
    private switchService: SwitchService
  ) {
    from(this.sensorModel.find()).subscribe(sensors => {
  ...
  deleteSensor(sensorId: string): Observable<void> {
    return from(this.sensorModel.findOneAndDelete({ _id: sensorId })).pipe(
      switchMap(deletedSensor =>
        this.hostService
          .deleteHost(deletedSensor.host)
          .pipe(
            switchMap(host => this.httpService.delete<void>(`http://${host.ip}:${host.port}/api/sensor/${deletedSensor.pin}`).pipe(map(() => {})))
          )
      )
    );
  }
  ...
  updateState(name: string, pin: string, state: State): Observable<void> {
    return from(
      this.sensorModel.findOne({ pin }, null, {
        path: 'host',
        match: { name }
      })
    ).pipe(switchMap(foundSensor => this.switchService.changeState(foundSensor.targetId, state)));
  }

  addTarget(sensorId: string, targetId: string): Observable<SensorModel> {
    return from(this.sensorModel.findOneAndUpdate({ _id: sensorId }, { targetId }));
  }

  private setCorrectSensorState(sensorId: string): void {
    from(this.sensorModel.findOne({ _id: sensorId }, null, { path: 'host' }))
      .pipe(
        switchMap(foundSensor =>
          this.httpService
            .get<StateResponse>(`http://${foundSensor.host.ip}:${foundSensor.host.port}/api/sensor/state/${foundSensor.pin}`)
